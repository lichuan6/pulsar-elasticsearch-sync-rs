        ...
        Self {
            operation: BulkOperation {
                header: BulkHeader {
                    ...
                    metadata: BulkMetadata { ..Default::default() },
                },
                source: Some(source),
            },
        }
    ...
    B: serde::Serialize,
{
    ...
    pub fn retry_on_conflict(mut self, retry_on_conflict: i32) -> Self {
        self.operation.header.metadata.retry_on_conflict =
            Some(retry_on_conflict);
        self
    }
...
mod tests {
    ...
    fn serialize_bulk_operations_with_same_type_writes_to_bytes(
        ...
        expected
            .put_slice(b"{\"update\":{\"_id\":\"4\",\"_source\":\"baz\"}}\n");
        expected.put_slice(b"{\"baz\":\"update_2\"}\n");
        expected
            .put_slice(b"{\"update\":{\"_id\":\"5\",\"_source\":[\"baz\"]}}\n");
